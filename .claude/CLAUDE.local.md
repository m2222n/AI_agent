...
**[남음] 다음 PR 후보 (우선순위순):**
1. **사이드바 신설** (현재 SaaS엔 사이드바 자체가 없음) — Streamlit sidebar.py: 데이터현황(ETF/주식 종목수+기준일), ETF/주식 목록(거래대금 TOP20, 등락률 색상, 종목검색, 섹터필터), 투자유의사항, 방문자카운터. **가장 큰 누락.** 백엔드: 종목 리스트 주는 엔드포인트 필요(movers 확장 or 신규).
2. **기술 분석 탭**: ❌ 장중 시세 차트(yfinance 15분봉 — generate_intraday_chart, 백엔드 엔드포인트 신설 필요) ❌ 골든/데드크로스 표시 ❌ 11개 지표 전부 상세(현재 일부만: 종가/추세/RSI/MACD/MA/볼린저%B — 스토캐스틱/일목/CCI/ADX/OBV/ATR 누락).
...
### 다음
- 위 체크리스트 1번(사이드바)부터. **F-2 KIS**(신분증), **푸시**(VAPID), 유료 전환($5/월~), 블로그 9편.

---

_Last Updated: 2026-06-10 (채팅 패리티 보강 — 동적 추천질문/피드백/재시도. SaaS↔Streamlit 기능 격차 체크리스트 작성. 백엔드 테스트 687개. 라이브 운영 중. Streamlit 병행)_
