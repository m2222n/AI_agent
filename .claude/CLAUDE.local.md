...
**[남음] 다음 PR 후보 (우선순위순):**
1. **비교 탭**: ❌ 수익률 막대차트(st.bar_chart→Recharts나 간단 바) ❌ 최근 분기 실적 비교표.
2. **재무 탭**: ❌ 연도 범위 슬라이더(현재 quarters 고정 12).
3. **전망 탭**: Prophet 축 표시 보강(현재 4축 카드 있으나 Prophet 미강조).
4. **탭 간 네비 버튼**(기술→재무/전망/채팅 바로가기), 데이터 범위 안내문구.

**참고**: Streamlit 전수 인벤토리는 이 세션 탐색 결과(167개).

### 다음
- 위 [남음] 1번(비교 탭 막대차트/분기실적)부터. **F-2 KIS**(신분증), **푸시**(VAPID), 유료 전환($5/월~), 블로그 9편.

---

_Last Updated: 2026-06-10 (Streamlit 패리티 보강: 채팅(추천질문/피드백/재시도) + 사이드바 + 기술탭(11지표/크로스/장중차트). /tabs/intraday·overview·movers·feedback. 백엔드 테스트 689개. 라이브 운영 중. Streamlit 병행. 남은 격차: 비교/재무 탭)_
