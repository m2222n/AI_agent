...
4. **섹터분석 탭 (보류)**: 합의=1일 디폴트 + 기간 선택 시 **그 기간~현재 시계열 차트**(+구간 수익률), 옵션 1일~10년+직접설정. **현황**: `_sector_blocking`→`_build_sector_stats`가 `_sector_index`(당일 스냅샷) `change_pct`(1일)만 시총가중. historical은 `daily_prices`(8.8M행 2014~)에 있음. ⚠️**보류=성능**: 최대 섹터 전기·전자 **314종목**, 1년 IN절 배치쿼리만 **2.2초**→10년 수십초. **결정 필요: 시총 상위 N(20/50) 지수 vs 전종목+캐싱.** 섹터 시계열 차트 함수도 신규 작성 필요(sector.py엔 overview/detail 바차트만).

### 다음
- (사용자) Railway 재배포 로그 검증(`준비 완료 (postgresql)` + 회원 유지) → 섹터 기간차트(#4, 지수구성 결정 후) → 가상투자 탭 → 후원("해줘" 시) → 블로그 9편 / 메일 인프라(비번찾기)

---

_Last Updated: 2026-06-18 (UI 개선 "하나씩" — #1 티커→종목코드 #64 / #2 재무제표 주식만+10년+직접설정 #65 / #3 비교분석 기간 직접설정 #66 머지. #4 섹터 기간차트는 성능(전기전자 314종목·10년) 트레이드오프 결정 필요해 보류. 직전: 계정기능(비번변경+닉네임+탈퇴) #63 + Railway 유저DB Postgres 영구화(재배포 로그 검증 남음). 다음: 섹터 #4 지수구성 결정 후 / 가상투자 탭 / 후원)_
