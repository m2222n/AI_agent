...
**[완료] 사이드바 신설 (2026-06-10):**
- ✅ `/tabs/overview`(데이터현황 etf/stock 개수+기준일 + 거래대금 TOP + 섹터목록) + Sidebar 컴포넌트.
- ✅ 데이터현황, ETF/주식 탭, 종목 검색, 거래대금 TOP20(등락률 색상, 클릭→/technical), 투자유의사항.
- layout: NavBar 아래 [Sidebar|content] flex, 데스크톱(lg) 좌측 / 모바일 숨김.
- ⏭️ 섹터필터(드롭다운) — 검색으로 갈음, 후속. ⏭️ 방문자카운터 — Supabase 연동 후속(저가치). ⏭️ 모바일 사이드바(드로어) — 후속.

**[남음] 다음 PR 후보 (우선순위순):**
1. **기술 분석 탭**: ❌ 장중 시세 차트(yfinance 15분봉 — generate_intraday_chart, 백엔드 엔드포인트 신설 필요) ❌ 골든/데드크로스 표시 ❌ 11개 지표 전부 상세(현재 일부만: 종가/추세/RSI/MACD/MA/볼린저%B — 스토캐스틱/일목/CCI/ADX/OBV/ATR 누락).
3. **비교 탭**: ❌ 수익률 막대차트(st.bar_chart→Recharts나 간단 바) ❌ 최근 분기 실적 비교표.
4. **재무 탭**: ❌ 연도 범위 슬라이더(현재 quarters 고정 12).
5. **전망 탭**: Prophet 축 표시 보강(현재 4축 카드 있으나 Prophet 미강조).
6. **탭 간 네비 버튼**(기술→재무/전망/채팅 바로가기), 데이터 범위 안내문구.

**참고**: Streamlit 전수 인벤토리는 이 세션 탐색 결과(167개). 사이드바가 단일 최대 격차.

### 다음
- 위 [남음] 1번(기술 분석 탭 장중차트/지표상세)부터. **F-2 KIS**(신분증), **푸시**(VAPID), 유료 전환($5/월~), 블로그 9편.

---

_Last Updated: 2026-06-10 (사이드바 신설(데이터현황+종목목록/검색) + 채팅 패리티 보강. /tabs/overview·movers·feedback 추가. 백엔드 테스트 688개. 라이브 운영 중. Streamlit 병행)_
