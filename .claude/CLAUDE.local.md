...
## iOS 앱화 — Capacitor 래핑 (2026-07-15, 브랜치 feat/ios-capacitor, 미머지)
상세 메모리: `memory/project_ai_agent_ios_app.md`. 빌드 가이드: `frontend/IOS_APP.md`.
- **계기**: 친구 재우(IT 비전공자)가 관심 → 함께 개발하며 배우고 정태민은 포트폴리오. 둘 다 아이폰+맥북. 목표 스토어 **Apple만**($99/년). 앱명 후보 주선생(확정·변경가능)/주교수/영차. 범위 1~5단계(시뮬레이터까지, 비용$0), 제출(6단계) 별도.
...
- **3단계 CORS**: `api/main.py`에 `_capacitor_origin_regex()` 헬퍼 — CORS_ORIGINS가 특정 origin으로 제한돼도 `capacitor://localhost`를 regex로 허용(WKWebView origin). 인증이 Authorization 헤더라 credentials 무관. **Python 3.9라 `list[str]`/`str|None` 금지** → `List`/`Optional`(import 추가). 테스트 3개(regex/wildcard-None/preflight importlib.reload 통합).
...
- **검증**: build:static(out/) + standalone(server.js, 웹 회귀 없음) 양쪽 OK. 백엔드 56+프론트 Vitest 17 통과. `cap sync ios` 성공. 커밋 2개(feat+docs).
- **5단계(사용자 몫)**: 이 Mac은 **Command Line Tools만, 전체 Xcode 없음**(App Store ~7GB). 절차 `frontend/IOS_APP.md`: Xcode설치→`xcode-select -s`→build:static→cap sync→cap open→시뮬 Run+체크리스트. **6단계 제출 전**: 아이콘 1024원본(현 512 업스케일)·번들ID 확정(임시 `com.example.etfrag`)·앱명 확정·Apple $99.

---

_Last Updated: 2026-07-15 (iOS 앱화 착수 — 친구 재우와 함께, Apple만 목표. Capacitor v8 래핑 1~4단계 완료: next.config 조건부 export/standalone·server.url無로 4.2 회피·**SPM이라 CocoaPods불필요**·manifest force-static·OfflineBanner/NativeAppInit·CORS capacitor origin regex 허용+테스트3. 앱명 "주선생". 브랜치 feat/ios-capacitor 미머지. 검증 build양쪽+백56/프17 green. 5단계는 Xcode 설치 후, 가이드 frontend/IOS_APP.md. 상세: memory/project_ai_agent_ios_app)_
_2026-07-10 세션후반3 (PR #114~116 — 가입자통계 /admin/stats[라이브 가입자 1명·방문242] + 성별 필수수집[Alembic 첫 실전 DDL, gender 라이브 컬럼추가 성공] + 비밀번호 재설정[JWT토큰·Resend메일]. **Resend 무료 도메인 제약**: onboarding@resend.dev는 가입계정 본인만 발송(타 수신자 403), 도메인 인증해야 전체발송 — 지금은 UI만 "준비중" 안내로 되돌리고 코드 보존. 907 green. 상세: memory/project_ai_agent_gender_pwreset_20260710)_ (PR #114~116 — 가입자통계 /admin/stats[라이브 가입자 1명·방문242] + 성별 필수수집[Alembic 첫 실전 DDL, gender 라이브 컬럼추가 성공] + 비밀번호 재설정[JWT토큰·Resend메일]. **Resend 무료 도메인 제약**: onboarding@resend.dev는 가입계정 본인만 발송(타 수신자 403), 도메인 인증해야 전체발송 — 지금은 UI만 "준비중" 안내로 되돌리고 코드 보존. 907 green. 상세: memory/project_ai_agent_gender_pwreset_20260710)_
