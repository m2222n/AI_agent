...
### 다음 (남은 것)
- 아래 기능 격차 체크리스트 + F-2 KIS/푸시/유료전환.

## SaaS ↔ Streamlit 기능 격차 체크리스트 (2026-06-10)

사용자가 "Streamlit에 있는데 SaaS에 빠진 것 동일 구현" 요청 → Streamlit UI(src/ui/, 167개 기능) 전수 대조. **우선순위대로 한 벌씩** 진행 합의. 비로그인도 전 기능(로그인 선택제).

**[완료] 채팅 보강 (2026-06-10):**
- ✅ 동적 추천질문(빈 채팅) — `/tabs/movers`(급등/급락/거래대금 TOP) + ExampleQuestions. 데이터 없으면 하드코딩 fallback.
- ✅ 피드백 👍/👎+부정사유 — `/feedback`(익명/로그인) + FeedbackButtons.
- ✅ 에러 재시도 버튼 — 마지막 user 질문 재전송.
...
**[완료] 재무 탭 조회 기간 선택 (2026-06-10):** 1년/2년/3년/5년(4/8/12/20분기) 세그먼트 버튼 → 선택 시 `getFinancial(ticker, quarters)` 재조회. 종목·기간 state 분리(기간 변경해도 종목 유지). 백엔드 `quarters` 1~200 허용이라 변경 불필요.

**[완료] 전망 탭 Prophet/통계 축 보강 (2026-06-10):** 통계·Prophet 축은 `predicted_return`/`confidence_interval`/`trend` 형태라 기존 `signal`+`key_factors` 렌더로는 **빈 카드**였음(=Prophet 미강조의 실제 원인). `forecastView()` 추가로 예측 수익률·신뢰구간·추세 렌더 + Prophet 카드 보라색 강조(📈). available=false면 "데이터 부족" 표시.

**[완료] 데이터 범위 안내문 (2026-06-10):** `DataRangeNote` 공통 컴포넌트(일별 시세 2014년 4월부터 보유 + 종목별 상장일 차이 + 면책) → 5개 분석 탭(기술/재무/비교/전망/섹터) 하단에 배치. 전망 탭은 기존 중복 면책 문구를 이 컴포넌트로 통합. **탭 간 네비**는 상단 NavBar가 이미 전 탭 전환 제공 → 컨텍스트 점프 버튼은 가치 낮아 보류(종목 state cross-page 전달 복잡도 대비).

**[2차 대조] (2026-06-10):** 첫 인벤토리 누락분 — Explore 2개로 Streamlit↔SaaS 재대조 결과 4건 추가 발견: (1) 방문자 카운터(Streamlit 사이드바엔 있으나 SaaS 없음), (2) 비교 탭 기간 선택(Streamlit selectbox, SaaS 고정 120일), (3) 기술 탭 10년 옵션 누락(Streamlit 5개 vs SaaS 4개), (4) 사이드바 ETF/주식 분리+업종필터. 사용자 요청으로 4건 전부 구현 진행.
...
### 후속: 현대차 섹터 AR 프롬프트 개선 (2026-06-18, PR #62 머지)

위 5차에서 "현대차 섹터" 빈 응답(별칭 미스)은 해소됐으나 AR=0.28 잔존 — 원인은 빈 응답이 아니라 **답변 구조**였음. `analyze_sector` 종목 경로가 보유 ETF 목록(최대 15개)을 먼저 길게 출력하고 정작 핵심인 `[업종]`+밸류에이션 백분위 위치를 맨 뒤에 둬서, "섹터 내 밸류에이션 위치" 질문의 핵심 정보가 묻혔음.

- `analyze_sector` 종목 경로: `[업종]`/밸류에이션 백분위를 **답변 상단**으로, 보유 ETF 목록을 그 뒤로 재배치.
- docstring에 "특정 종목의 업종 내 상대 위치를 물으면 종목명으로 호출 → 그 종목의 업종/PER/PBR/배당/시총 백분위 반환. 섹터명으로 호출하면 종목 목록만 나옴" 안내 추가(LLM 인자 선택 개선).
- 회귀 테스트 `test_analyze_sector_valuation_before_etf_list`(`[업종]`이 `보유한 ETF`보다 앞서는지). sector 테스트 28개 + 전체(API 제외) 708개 통과.
- 브랜치 sector-valuation-ordering → PR #62 squash 머지.
- **RAGAS 재측정(`run_eval.py --types sector`, eval_20260618_151255)**: sector AR **0.845**(이전 5차 신규20개 합산 0.501, 그중 현대차 0.28이 발목이었음), F 0.946, CR 1.000, Hit Rate 100%(6/6). **답변 구조 재배치만으로 sector 답변 관련성이 크게 올랐음을 정량 확인.** 교훈: 빈 응답이 아니어도 핵심 정보의 답변 내 위치가 RAGAS AR을 좌우함.

### 코드 점검 라운드 (PR #58/#61/#62) — 2026-06-18

PR #57 이후 머지된 변경(F-3 로컬 감성 #58 + RAG 품질 #61 + sector 재배치 #62, 코드 559줄)을 점검. 직전 6-04/6-15 교훈("LLM 오판 기각, 실제 코드 검증분만") 적용 — 의심점 4개를 실제로 검증하고 모두 안전 확인. **실버그 0건.**

- `sentiment._to_korean`: `LABEL_n`/소문자/대문자 라벨 전부 2단계 `.get()` 조회로 커버 ✓
- `news._fetch_feed`: certifi 직접 fetch 실패 시 feedparser 기본 경로 fallback, 이중 try/except ✓
- `analysis` 별칭 가드: 종목명("현대차")이 짧은 별칭("차")에 안 끌려감 — 전체 일치 `.get()`만 + `is_known_stock` 가드 ✓
...
### UI 개선 요청 4건 (2026-06-18, 사용자 "하나씩") — #1~3 완료, #4 보류
1. **'티커'→'종목코드' (PR #64)**: 화면 노출 문구만 교체(컴포넌트명/변수명 코드 식별자는 유지). 프론트 TickerSearch 기본 placeholder + 재무 placeholder, Streamlit `_ticker_input` placeholder + 기술분석 caption.
2. **재무제표 탭 (PR #65)**: ETF 재무제표 없음 → **주식만 자동완성**(`get_available_tickers(asset_type)` + `/tabs/tickers` asset_type 쿼리 `^(stock|etf)$` + `searchTickers(q,limit,assetType)` + `TickerSearch` assetType prop, 재무탭 assetType="stock") + **10년(40분기)** + **직접설정**(년→분기 ×4, 1~12년). 테스트 api_tabs 29→31.
3. **비교분석 탭 (PR #66)**: 상대수익률 기간 **직접설정**(년→영업일 ×250, 1~10년). 백엔드 days 재사용(변경0).
4. **섹터분석 탭 (PR #68 완료)**: 1일 디폴트 스냅샷 유지 + 업종 선택 후 기간(1주~10년) 선택 시 **그 기간~현재 시총가중 지수 시계열 차트**+구간 수익률. **결정=시총 상위 20종목 가중지수**(대형주 지배라 대표성 충분, 전기전자 314종목도 ~0.6초). `get_closes_batch`(IN절 배치) + `_sector_trend`(기준일=100, 중간상장 제외) + `generate_sector_trend_chart` + `/tabs/sector?period=`. 직접설정은 백엔드 고정옵션이라 제외(1일~10년 버튼). 테스트 9개. **데이터 특성**: 이 환경 주가가 1년 수백% 등락(가상/시뮬값 추정)이나 계산은 정합(가중평균∈구성종목 수익률 범위).

### 사용자 검증 완료 (2026-06-19~23)
- **유저DB 영구화**: curl 라이브 검증(회원가입201→로그인→탈퇴204→재로그인401) 성공. 계정/관심종목 재배포해도 보존.
...
### 영문 대문자 포함 티커 조회 실패 실버그 (2026-06-24, PR #74)
사용자 발견: 0162Z0(RISE 삼성전자SK하이닉스채권혼합50) 자동완성엔 뜨나 기술분석 "데이터 없음". #72(시세부족)인 줄 알았으나 **시세 82행 충분 → 진짜 버그**. **원인: 신형 ETF 티커 영문 대문자 포함(0162Z0/0192L0)인데 `_state._build_data_index`가 티커를 원본대소문자로 키(`index[ticker]`) 저장, `_find_structured_data`는 query `.lower()` 조회 → '0162z0'≠'0162Z0' 정확·부분 매칭 모두 실패→None.** 이름조회는 됐으나 자동완성은 티커를 넘겨 실패(숫자만 티커는 무영향이라 미발견).
- 수정: `_build_data_index` `index[ticker.lower()]=item`(이름은 이미 .lower()) + `_helpers._fallback_deploy_lookup` 티커 비교도 `.lower()`.
- 회귀 테스트 `test_find_structured_data.py` 5개(대/소문자 티커·숫자 티커·이름·미존재). 전체 833.
- **교훈: 티커에 영문 포함 종목 있음 → 인덱스 키와 조회 키의 정규화(.lower()) 일치 필수.** 티커로 해석하는 전 경로(기술/전망/비교/가상투자) 영향.

### 가상투자 주문 UX (2026-06-24, PR #75)
종목 선택 시 현재가 + 전날대비 등락률(괄호, signColor) 표시(`getPrice`/tabs/price 재사용, 장외 '종가' 배지) + 금액 비율 버튼 10/25/50/100%(`setQtyByRatio`: 현금×비율/현재가 floor) + 예상 체결금액(수량×현재가). 프론트 전용(invest/page.tsx), 백엔드 변경 없음. tsc 통과.
...
## ToDo 1~10 일괄 처리 완료 (2026-06-26, PR #88~#92)
사용자 "1~10 순서대로 다 하자" 지시로 하루에 처리. **상세: `memory/project_ai_agent_todo_round_20260626.md`.**
- ①doc정리 ②MEMORY 43KB→8KB(PR별 상세 timeline_detail.md 분리)
- ③④ **env 진단 스크립트(PR #88)** `scripts/check_prod_env.py` — 진단: VAPID 미등록, KIS 장중확인필요, CRON 정상. 등록은 사용자 Railway 작업
- ⑤ **KOSDAQ 백필(PR #89)** — 진짜원인=KRX 로그인 누락(외부장애 아님). ensure_krx_login 추가→KOSPI2096+KOSDAQ1822. 빈값797=상폐
- ⑥ **F-3 로컬감성 = GPT fallback 유지 결정**(Railway 메모리 제약, 코드 변경 0)
- ⑦ **관심종목⭐ 재무/전망/비교 탭 확대(PR #90)**
- ⑧ **가상투자 고도화(PR #91)** — 비중 SVG 도넛(PortfolioPie 프론트전용) + 거래통계(/stats 승률·손익비) + 배당정산(/dividend 연DPS×수량 라운드당1회, last_dividend_at 마이그레이션, get_latest_dps). 배당 데이터한계=pykrx DPS는 TTM 반복값이라 배당락 없음→예상연간배당 수동정산
- ⑨ **RAGAS 재측정** — forecast+news 14개 실측 **F=0.988·CR=1.000·Hit100%·AR=0.711**(eval_20260626_113538). AR은 요약형 역질문 측정한계가 평균 누름(도구 정상). 코드 수정 불필요
...
### 나이대(선택) 수집 + ID/비밀번호 찾기 안내 (PR #101)
- **나이대(age_group)**: 사용자 요청 — 회원가입 시 나이대만(이름·성별 X). **개인정보 최소화** 판단: 분류정보(10대~70대 이상 7버킷)라 식별 불가·PIPA 부담 낮음, 선택 입력. 맞춤 추천 대비.
  - `User.age_group` 컬럼 + 멱등 마이그(nickname 패턴). Signup/ProfileUpdate/UserResponse에 age_group(허용값만 저장·그 외 무시, profile은 None=미변경). `AGE_GROUPS` 상수 백엔드·프론트 일치.
...
## 모바일 앱화 — Capacitor 래핑 iOS+Android (2026-07-15, 브랜치 feat/ios-capacitor, 미머지)
상세 메모리: `memory/project_ai_agent_ios_app.md`. 빌드 가이드: `frontend/IOS_APP.md`(iOS+Android 섹션).
- **계기**: 친구 재우(IT 비전공자, Windows 노트북)가 관심 → "빨리 올려 홍보·피드백" 목적(그래서 투자도). 정태민은 포트폴리오. 앱명 후보 주선생(확정·변경가능)/주교수/영차.
- **⚠️ 장비 제약(방향을 결정한 핵심)**: 태민 맥 = **MacBook Pro 16" 2019 Intel i7, macOS 14.8.7, 회사 업무용**. macOS 업그레이드 함부로 불가(업무툴/VPN/보안 깨질 위험·롤백 어려움). 최신 Xcode=macOS 15↑ 필요 → **iOS 시뮬레이터/빌드 지금 불가**(이 맥 상한은 macOS 15, 그 위 macOS 26은 Intel 지원종료). 재우는 Windows라 iOS 개발 불가(애플 정책). 둘 다 아이폰 유저. **맥 업글은 6000 회사서버와 무관**(SSH 클라이언트일 뿐).
- **PWA 이해(사용자와 정리)**: "홈화면 추가" = 라이브 URL(`radiant-abundance...`) 전체화면 실행일 뿐, 기능은 URL과 동일. 둘이 쓰기엔 PWA/URL로 충분하나 홍보·검색노출·푸시는 App Store 앱만의 강점. 라이브 URL 정상(프론트/백 200 확인).
- **결정(방향 전환)**: ①지금은 **URL/PWA로 홍보 시작**(무료·당장, 재우 메시지 초안 세션에 있음) ②**안드로이드 앱 우선**(이 맥+Android Studio로 가능, macOS 업글·윈도우·공기계 다 불필요, 에뮬레이터 테스트) ③iOS는 개인맥/새맥 생기면. Google Play $25(평생1회) vs Apple $99/년.
- **설계(Apple 4.2 회피)**: 원격 URL 래퍼 금지 — Next.js `output:"export"` 정적 번들을 기기 내장(`capacitor.config.ts` webDir=out, **server.url 미설정**), 데이터만 라이브 백엔드 fetch. 앱이 순수 클라 SPA(모든 페이지 `'use client'`, SSR/서버컴포넌트/route.ts/next-headers 전무, 이미지 일반 `<img>`)라 export 전환 무리 없음.
- **1단계 export 전환**: `next.config.ts` — `BUILD_TARGET=static`이면 `output:"export"`, 기본 `standalone` 유지 → **웹(Railway)/앱 한 코드베이스**. `images.unoptimized:true`. `package.json` `build:static`이 라이브 백엔드 URL 빌드타임 주입(`.env.local`의 localhost 이김 확인). **함정: `manifest.ts`에 `export const dynamic="force-static"` 필수** — 없으면 `/manifest.webmanifest` 동적 라우트가 export 빌드를 막음(standalone엔 무해). 15라우트 전부 정적 프리렌더, Suspense 이슈 없었음(Next16이 useSearchParams 자동 처리).
- **2단계 Capacitor**: v8(8.4.2). `npx cap add ios` → **SPM 기반이라 CocoaPods 불필요**(v8 특징, Package.swift로 플러그인 통합). 플러그인 4개(app/network/splash-screen/status-bar) v8 통일. `ios/.gitignore`가 빌드산출물(App/App/public·DerivedData·Pods) 자동 제외 → Xcode 프로젝트·SPM·자산 25파일만 커밋.
- **3단계 CORS**: `api/main.py`에 `_capacitor_origin_regex()` 헬퍼 — CORS_ORIGINS가 특정 origin으로 제한돼도 Capacitor 앱 origin을 regex로 허용. **iOS는 `capacitor://localhost`, Android WebView는 `http://localhost`(포트 없음)** 로 서로 달라 regex `^(capacitor://localhost|http://localhost)$`로 둘 다 매칭(Next dev `:3000`은 포트로 구분·제외). 인증이 Authorization 헤더라 credentials 무관. **Python 3.9라 `list[str]`/`str|None` 금지** → `List`/`Optional`. 테스트 3개.
- **4단계 4.2 마감**: `OfflineBanner.tsx`(네이티브=@capacitor/network, 웹=navigator.onLine → 오프라인 시 흰화면 대신 배너, safe-area-inset 대응) + `NativeAppInit.tsx`(StatusBar 스타일+SplashScreen.hide, `Capacitor.isNativePlatform()` 가드 + 동적 import로 웹 번들 오염 방지) + `layout.tsx` 배선 + 앱명 "주선생"(metadata/manifest). 아이콘/스플래시 `@capacitor/assets generate --ios`(소스 `frontend/assets/icon.png`=기존 512 복사).
...
1. ~~`chore/android-signing-prep` 브랜치 푸시·머지~~ → **2026-08-20 완료** (PR #123 머지 `3083cb2`, CI green test 2m38s·rag-eval pass, 브랜치 삭제, origin/main 시크릿 0 확인).
...
3. 스크린샷(에뮬 `adb exec-out screencap -p > shot.png`, Play용 폰 2장+와 겸용) → Play $25 등록 → **비공개 테스트 트랙 업로드** → 테스터 12명 14일 → 프로덕션 승격.
4. 블로그 9편 집필. 스크린샷 체크리스트는 위 "📸 블로그 9편" 섹션 유지.

---

## 🔑 테스터 12명/14일 요건 — 조사 결과 (2026-08-21, 공식문서+실무자료 확인)

**대상**: 2023-11-13 이후 생성된 **개인** 개발자 계정(조직 계정은 면제). 태민님은 개인 계정이라 해당.

### 형식 요건 vs 실질 심사 (2층 구조 — 이게 핵심)
| 층 | 내용 | 위반 시 |
|---|---|---|
| **형식 요건**(공식 문서 문자) | 12명이 **14일 연속 옵트인 유지** | 카운터 리셋 |
| **실질 심사**(프로덕션 승격 신청서) | 테스터가 **실제로 앱을 썼는지**를 Play Console 통계와 대조 | "더 테스트하세요" 반려 |

- **공식 문서는 "opted in continuously"만 요구**하고 사용 의무를 명시하지 않음. 그러나 14일 후 **프로덕션 접근 신청서**에서 "테스터가 모든 기능을 써봤는지", "사용 패턴이 실제 프로덕션 유저와 비슷했는지"를 묻고 **심사자가 답변을 Console 실제 지표와 대조**함.
- **실무 결론: 설치만으론 반려됨.** 최다 반려 사유가 **"insufficient testing engagement"**(옵트인했지만 앱을 안 연 테스터). "12명 옵트인 + 세션 0"은 확정 반려.
- **안전 기준: 14일간 매일(최소 이틀에 한 번) 앱 열기.**

### 리셋 함정
- **앱 삭제(uninstall) → 옵트인 카운트 감소 → 14일 시계 리셋.** 가장 치명적.
- 설치했지만 안 열기 → 카운트는 유지되나 engagement 그래프가 평평해져 승격 심사에서 반려.
- **12명이 같은 14일 창에 겹쳐야 함**(누적 12명 ❌). 7일차에 1명 이탈 → 12명 미달 → 리셋.
- → **12명이 아니라 15~16명 여유 있게 모으는 게 정석.**

### 지인에게 부탁할 때 문구
"앱 깔아만 놔줘" ❌ → **"2주 동안 하루에 한 번만 앱 열어서 종목 하나 검색해줘. 그리고 2주 동안 절대 지우지 마."** ⭕
- 주식 앱이라 유리: "관심종목 등록해두고 장 열릴 때 확인해봐"가 자연스러운 사용 패턴으로 포장됨.

### ❌ 유료 대행 업체 (쓰지 말 것)
- 시장 존재함($15~30, 몇 시간~며칠 내 12명): testerscommunity.com, 20apptester.com, primetestlab.com, testfi.app, 12testers14days.pro 등.
- **리스크가 비대칭**: Google이 2026년 집중 단속 중 — Play Services가 하드웨어를 깊이 검사(x86을 ARM으로 위장, 배터리 무감소, 가속도계 데이터 부재)해 에뮬레이터를 플래그. 걸리면 **프로덕션 접근 거부 + 개발자 계정 영구 정지**. 업체가 에뮬 농장을 쓰는지 발주자는 확인 불가인데 대가는 발주자가 치름 → **$20 아끼려 계정을 거는 거래**.
- 게다가 대행 테스터는 앱에 관심 없어 최소한만 하거나 안 함 → **engagement 부족으로 반려**되는 최악 조합이 흔함.
- 목적(포트폴리오·완결 경험)과도 불일치 — 블로그/면접에 쓸 대목이 아님.

### ⚠️ 품앗이(상호 테스트) — 태민님은 참여 불가
- 존재함: 네이버 카페(안드로이드 개발자), 오픈카톡방("구글 플레이 테스터"), Reddit `r/androiddev`·`r/AndroidClosedTesting`, Discord 인디 개발자 서버. **유료 대행과 성격이 다르고 정책상 문제없음**(실제 사람+실제 기기라 Google이 잡으려는 대상이 아님, 상대도 자기 앱을 통과시켜야 해서 성실).
- **그런데 태민님은 아이폰 유저 → 남의 안드로이드 앱을 설치할 기기가 없음 → 갚을 수가 없음.** 에뮬레이터는 카운트 안 됨(하드웨어 검사). 받는 쪽만 하는 건 커뮤니티가 걸러내고, 결국 유료 대행과 같은 위치로 회귀.
- → **태민님의 유일한 정직한 경로 = 갤럭시 쓰는 지인 12명.**

### 동원 가능 풀 — ⚠️ 매우 제한적 (2026-08-21 사용자 확인)
**사용자 확인 사항: 대학 동기·선후배 없음, 재우 없음.** 앞서 이 둘을 풀에 넣었던 것은 잘못된 가정.
- 남은 지인 풀 = **회사(오리누) 동료 + 가족·친척** 정도. 그마저 갤럭시 사용자로 한정되면 **12명은 사실상 불가능에 가까움.**
- → **품앗이 커뮤니티가 주력이어야 하고, 지인은 보조**로 봐야 함(원래 판단의 반대). 아래 "모집 채널" 참조.

### 📍 모집 채널 조사 (2026-08-21) — 실행 순서
**⚠️ 직전 판단 정정**: "아이폰 유저라 품앗이 참여 불가"라고 단정했으나, **테스트잇에 안드로이드 폰 없이 크레딧 버는 경로가 있음**을 확인. 품앗이가 완전히 닫힌 게 아니었음.

| 순위 | 채널 | 비용 | 아이폰 유저 가능? | 비고 |
|---|---|---|---|---|
| ⭐1 | **네이버 카페** `cafe.naver.com/devsharing` ("안드로이드 비공개 테스터" 검색) | 무료 | 협의 여지 | **반나절에 12명 전원 모집 실증** |
| ⭐2 | **테스트잇** `test.it.kr` | 무료(가입 1,000P) | **⭕ 웹사이트 테스트로 크레딧** | **B 연속형 필수** |
| 3 | 카톡 오픈채팅 / `테스터 품앗이` 앱 / LaunchPad(apptesters.cc) | 무료 | 채널별 | LaunchPad는 초기단계(등록 앱 0건) |
| 4 | Reddit `r/AndroidClosedTesting`·`r/androiddev`·Discord | 무료 | 상호교환 필요 | 영어·품질 편차 |
| ❌ | 크몽·Fiverr·Upwork 유료 대행 | $15~30 | — | 저가는 에뮬·봇 → **계정 정지** |

- **테스트잇이 핵심 발견**: 크레딧 기반 품앗이인데 **웹사이트 테스트(PC에서 남의 사이트 검수)로 크레딧 획득 가능** → 안드로이드 기기 없이도 갚을 수 있음. 태민님 QA·버그리포팅 강점 활용 가능. **단 "B 연속형"(14일 3회 인증) 모집만 12/14 요건 충족** — A 일반형(1회)은 미충족. 플랫폼도 "중간 opt-out 리셋 → 2~3명 여유" 경고. 규모가 작아(앱 모집 4건 수준) 카페 병행 필요.
- **실행 권장(2026-08-21 수정)**: 지인 풀이 사실상 없으므로(위 참조) **품앗이 커뮤니티가 주력**. 네이버 카페 + 테스트잇을 **동시에** 돌려 15~16명 확보하고, 회사 동료·가족은 보조로 채움. 단일 채널 의존 금지(이탈 1명이면 리셋).
- **아이폰 유저 + 지인 없음 조합이므로 "갚을 수단"이 관건**: 테스트잇 웹사이트 테스트로 크레딧을 버는 경로가 실질적으로 유일한 자립 수단 → **여기가 가장 먼저 확인해야 할 채널**. 카페 품앗이도 "제 앱은 못 깔아드리지만 웹/QA로 돕겠다"는 식의 제안이 통할지 문의해볼 가치 있음.

### ⚠️ 실전 함정 (실제 사례)
- **"테스터 이메일 등록만 하면 된다"는 착각**: 어느 팀은 등록만 받고 끝냈다가 "**다운로드 수가 없으면 프로덕션 출시 권한 신청이 거절될 수 있다**"는 걸 뒤늦게 알고 전원에게 다시 연락해 설치를 부탁해야 했음([velog @bcgo99](https://velog.io/@bcgo99/안드로이드-앱-배포기)). → **등록 → 옵트인 → 설치 → 매일 사용**까지가 한 세트.
- 지인 부탁이 **심리적으로 부끄럽다**는 후기가 공통적 → 품앗이 커뮤니티는 서로 같은 처지라 이 부담이 없다는 게 실질적 장점.

### 내부 테스트 = 폴백 (포기 아님)
- 내부 테스트 트랙: **최대 100명, 12명/14일 요건 면제**. keystore·서명 AAB·Play Console 업로드·심사 통과·링크 배포 전부 경험 가능.
- **단 "Google Play 출시"라고 쓸 수 없음** — 이력서에서 "Play Console 등록·심사 통과"는 설명이 필요한 문장이고, 면접에서 "왜 정식 출시는 안 하셨어요?" 질문을 유발(답이 기술 이유가 아니라 인맥 이유라 궁색). 검색 노출·링크 수명도 차이.
- **⚠️ 2026-08-20 세션에서 "검색 노출 하나만 없다"고 정리한 것은 안이했음 — 사용자 지적으로 정정.** 실제 차이는 포트폴리오 문장의 급 차이.

### ✅ 결정 (2026-08-21): 프로덕션 정식 출시를 목표로 한다
사용자 의사: **"포트폴리오는 급하지 않지만 이거 자체를 제대로 끝내버리고 싶다."** → 내부 테스트 타협 없이 **프로덕션 승격까지 완주**.
- **핵심 통찰: 다음주 할 작업은 어느 쪽이든 동일하다.** keystore → 서명 AAB → Play $25 → 업로드까지는 프로덕션이든 내부테스트든 같음. 업로드를 **비공개 테스트(closed testing) 트랙**으로 하면 그 시점부터 12명 카운터가 돌기 시작 → **지금 결정할 필요 없이 업로드까지 밀어붙이면 됨.**
- **"14일 대기"와 "다른 프로젝트 개발"은 충돌하지 않음.** 14일은 대기 시간이고 매일 앱을 여는 건 지인들이 함. 태민님은 카운터만 가끔 확인. (2026-08-20 세션에서 이 둘을 대립시킨 것은 틀린 판단이었음 — 정정.)
- 병행 작업: **갤럭시 지인 명단 세어보기**(카톡 프로필로 대략 파악) → 15~16명 확보 목표.
- 정말 안 되면 그때 내부 테스트로 선회(손실 0). **$25는 평생 1회·유효기간 없음**이라 나중 승격도 가능.

---

_2026-08-21 (**테스터 12명/14일 요건 조사 + 프로덕션 정식 출시로 확정**). 코드 변경 없음(조사·의사결정 세션). **① PR #123 머지 완료**(`3083cb2`, CI green — test 2m38s·rag-eval pass, 브랜치 삭제, **origin/main 시크릿 0 실측 확인**: 템플릿만 추적·비번 필드 빈 값). **② 테스터 요건 = 2층 구조**(공식문서+실무자료 확인): 형식요건은 "12명 14일 연속 옵트인"뿐이지만, 14일 후 **프로덕션 접근 신청서**에서 "테스터가 모든 기능을 썼는지"를 묻고 **심사자가 Play Console 실제 지표와 대조** → **설치만으론 반려**(최다 반려사유 = "insufficient testing engagement", 옵트인+세션0은 확정 반려). **안전기준=14일간 매일 앱 열기**. 리셋 함정: **삭제 시 카운트 감소→시계 리셋**(가장 치명적), 12명이 **같은 14일 창에 겹쳐야**(누적❌, 7일차 1명 이탈→리셋) → **15~16명 여유 확보가 정석**. 부탁 문구는 "깔아만 놔줘"❌ → **"2주간 하루 한 번 열고 절대 지우지 마"**⭕(주식앱이라 "관심종목 등록하고 장 열릴 때 확인"으로 자연스럽게 포장). **③ 유료 대행 = 쓰지 않기로**: 시장 존재($15~30)하나 Google이 2026년 집중단속(하드웨어 검사로 에뮬 플래그 — x86 위장·배터리 무감소·가속도계 부재) → 걸리면 **계정 영구 정지**. 업체의 에뮬 사용 여부는 발주자가 확인 불가인데 대가는 발주자가 치름 = **$20 아끼려 계정 거는 거래**. 게다가 대행 테스터는 engagement 부족으로 반려되는 최악 조합 흔함. **④ 품앗이 = 정책상 문제없으나 태민님은 참여 불가**: 네이버카페/오픈카톡/Reddit `r/androiddev` 등에 존재하고 실제사람+실제기기라 단속 대상 아님(상대도 자기 앱 통과해야 해 성실). 그런데 **아이폰 유저라 남의 안드로이드 앱을 깔 기기가 없어 갚을 수 없음**(에뮬은 카운트 불가) → 받는 쪽만 하면 커뮤니티가 걸러내고 유료대행과 같은 위치로 회귀. **→ 유일한 정직한 경로 = 갤럭시 지인 12명**(한국 갤럭시 점유율 60~70%라 지인 20명 중 12~14명 가능성. 회사 오리누 동료·가족·대학동기·재우[하차했으나 테스터는 가능]). **⑤ ⚠️두 건 정정**: (a) 8-20에 내부테스트와 프로덕션 차이를 "검색 노출 하나뿐"이라 정리한 것은 **안이했음** — 사용자 지적대로 **"Google Play 출시"라 쓸 수 있느냐**의 급 차이이고, 내부테스트는 면접에서 "왜 정식출시 안 했나"를 유발(답이 인맥 이유라 궁색). (b) 8-20에 "14일 대기"와 "다른 개발 시작"을 대립시킨 것도 **틀림** — 14일은 대기시간이고 매일 여는 건 지인들이 하므로 **충돌하지 않음**. **⑥ ✅결정: 프로덕션 정식 출시 완주**(사용자: "포트폴리오는 급하지 않지만 이거 자체를 제대로 끝내버리고 싶다"). **핵심: 다음주 할 작업은 어느 쪽이든 동일** — keystore→서명AAB→Play $25→업로드까지 같고, **비공개 테스트 트랙으로 업로드하면 그 시점부터 12명 카운터가 돎** → 지금 결정 불필요, 업로드까지 밀어붙이면 됨. 병행: 갤럭시 지인 명단 세기(15~16명 목표). 안 되면 내부테스트 선회(손실0·$25 평생1회 유효기간 없음). 상세는 위 "🔑 테스터 12명/14일 요건" 섹션_

_2026-08-20 (**릴리스 서명 배선 + AAB 파이프라인 검증**, 브랜치 `chore/android-signing-prep` `7867fc7` → **PR #123으로 머지 완료** `3083cb2`). 17일 공백 후 복귀 — 그사이 커밋 17개는 **전부 자동수집**(신규 개발 0), 로컬 main이 17커밋 뒤여서 pull. **복귀 점검 전부 정상**: 라이브 백/프론트/`/privacy` 200, 데이터 최신일 **20260819**(ETF769·주식1843), 자동수집 **5/5 success** → **17일 무개입 운영 실증**(블로그 소재: "만들었다"가 아니라 "운영했다"). **① ⚠️보안 이슈 1건 실제 발견·수정**: Capacitor 템플릿 `.gitignore`는 `#*.jks`/`#*.keystore`가 **주석 처리**돼 있고 `key.properties`는 **항목 자체가 없었음** → 그대로 서명 배선했으면 **릴리스 서명 비번이 공개 리포에 커밋**될 수 있었음(유출 시 앱 위장 서명 가능·되돌릴 수 없는 사고). 주석 해제+`key.properties` 추가 후 **더미 파일로 `git check-ignore` 실측 검증**(가정 아님) + 커밋 직전 스테이징 grep으로 혼입 0 재확인. **② 조건부 서명 배선**: `key.properties` 있으면 릴리스 서명, **없으면 미서명 빌드로 폴백**(무조건 서명하면 keystore 없는 로컬/CI가 전부 깨짐). `key.properties.example` 신설(`storeFile`은 절대경로 — gradle은 `~` 미확장). **③ AAB 드라이런 성공**: build:static→cap sync→`bundleRelease` **BUILD SUCCESSFUL 57s**, **3.6MB**, appId `ai.jusunsaeng.app`·앱명 `주선생`·`/privacy` 포함 확인. ⚠️**AAB 무서명은 정상** — `META-INF/*.RSA` 없다고 서명실패로 의심했으나 AAB는 APK와 서명구조가 다르고 keystore 없으니 `signReleaseBundle`이 no-op, 즉 **디버그키로 몰래 서명 안 됐다는 뜻**(디버그 서명 AAB는 Play 거부)이라 의도된 동작. `base/root/META-INF/*.version`은 라이브러리 버전파일로 서명과 무관 — 오독 금지. **④ 아이콘 1024 생성**(512 업스케일 상태 해소). **⑤ 회귀 all green**: 백엔드 **911**·프론트 **22**·tsc0·`out/privacy.html`+백엔드URL 주입. **⑥ 사용자 질문 3건 판단(재논의 방지)**: 배포+블로그로 **마무리 찬성**(단 테스터 12명이 진짜 관문, 못 모으면 내부테스트+"제출까지의 전과정" 블로그로 완결) / GitHub **private 전환 반대**(포트폴리오는 남이 봐야 가치·시크릿은 이미 다 밖에 있고 유일 구멍인 keystore를 이번에 막음) / **리팩터링 불필요**(7-10 #110에서 했고 이후 신규코드 없음, 출시직전 리팩터링은 리스크만 — "빌드되는 상태를 건드리지 않는다"). 다음: 브랜치 푸시·머지 → **keystore(사용자 직접·📸1회성 캡처 먼저)** → 스크린샷 → Play $25 → 테스터 12명 14일 → AAB 제출 → 블로그 9편_
...
_Last Updated: 2026-07-15 (모바일 앱화 — Capacitor v8로 iOS+Android 래핑, 브랜치 feat/ios-capacitor 미머지. 설계: output:"export" 정적번들 기기내장·server.url無로 Apple 4.2 회피, 웹/앱 한 코드베이스(BUILD_TARGET). CORS는 iOS capacitor://localhost + Android http://localhost 둘 다 regex 허용. OfflineBanner/NativeAppInit/manifest force-static. 앱명 "주선생". **장비 제약**: 맥=업무용 Intel 2019 macOS14(업글 불가)→iOS 시뮬 막힘, 재우=Windows. **결정: URL/PWA 홍보 + 안드로이드 우선**(이 맥+Android Studio 가능·공기계/윈도우 불필요). 검증 build양쪽+백56/프17 green. 가이드 frontend/IOS_APP.md. 상세: memory/project_ai_agent_ios_app)_
_2026-07-10 세션후반3 (PR #114~116 — 가입자통계 /admin/stats[라이브 가입자 1명·방문242] + 성별 필수수집[Alembic 첫 실전 DDL, gender 라이브 컬럼추가 성공] + 비밀번호 재설정[JWT토큰·Resend메일]. **Resend 무료 도메인 제약**: onboarding@resend.dev는 가입계정 본인만 발송(타 수신자 403), 도메인 인증해야 전체발송 — 지금은 UI만 "준비중" 안내로 되돌리고 코드 보존. 907 green. 상세: memory/project_ai_agent_gender_pwreset_20260710)_
