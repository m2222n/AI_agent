...
**[남음] 다음 PR 후보 (우선순위순):**
1. **탭 간 네비 버튼**(기술→재무/전망/채팅 바로가기), 데이터 범위 안내문구.

**참고**: Streamlit 전수 인벤토리는 이 세션 탐색 결과(167개).

### 다음
- 위 [남음] 1번(탭 네비 버튼)부터. **F-2 KIS**(신분증), **푸시**(VAPID), 유료 전환($5/월~), 블로그 9편.

---

_Last Updated: 2026-06-10 (Streamlit 패리티 보강: 채팅 + 사이드바 + 기술탭(11지표/크로스/장중차트) + 비교탭(returns 버그수정/5기간 수익률/분기실적) + 재무탭(1~5년 기간) + 전망탭(Prophet/통계 축 예측수익률·신뢰구간 렌더, 빈 카드 버그 수정). 라이브 운영 중. Streamlit 병행. 남은 격차: 탭 네비 버튼)_
