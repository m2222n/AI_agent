...
_Last Updated: 2026-07-15 (모바일 앱화 — Capacitor v8로 iOS+Android 래핑, 브랜치 feat/ios-capacitor 미머지. output:"export" 정적번들 기기내장·server.url無로 Apple 4.2 회피·웹/앱 한 코드베이스(BUILD_TARGET)·SPM(CocoaPods불필요)·manifest force-static·OfflineBanner/NativeAppInit·CORS는 iOS capacitor://localhost+Android http://localhost 둘 다 regex. 앱명 "주선생". **장비 제약**: 맥=업무용 Intel 2019 macOS14(업글 불가)→iOS 시뮬 막힘·재우=Windows → **결정: URL/PWA 홍보+안드로이드 우선**(이 맥+Android Studio 가능·공기계/윈도우 불필요·Play $25, iOS는 개인맥 생기면). build 양쪽+백56/프17 green. 상세: CLAUDE.local.md·frontend/IOS_APP.md·memory/project_ai_agent_ios_app)_
