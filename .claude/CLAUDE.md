...
- [~] **Phase G: 모바일 앱 (iOS+Android, Capacitor 래핑)** — 2026-07-15 착수(브랜치 `feat/ios-capacitor`, 미머지). RN 재작성 대신 기존 Next.js 앱을 **Capacitor v8**로 래핑(코드 재작성 0). 계기: 친구 재우(Windows)와 함께, "빨리 올려 홍보". `output:"export"` 정적번들 기기내장(`server.url` 미설정 → Apple 4.2 회피) + OfflineBanner/상태바 네이티브 마감 + CORS는 iOS(capacitor://localhost)·Android(http://localhost) 둘 다 허용. 앱명 "주선생". **iOS·Android 프로젝트 코드 완료·검증 green.** ⚠️**장비 제약(2026-07-27 확정)**: 맥=업무용 Intel 2019, macOS는 Tahoe 26.5.2로 업글함. 그러나 **최신 Xcode 26.6은 Apple Silicon(M1+) 전용 → Intel 맥은 CPU 때문에 설치 불가**(App Store 실물 화면 "M1 이상 필요" 확인). 진짜 관문은 macOS 버전이 아니라 CPU였음(앞선 "Tahoe면 iOS 열림" 판단은 오류·정정). **이 맥으로 iOS 직접 빌드 영구 불가** → iOS는 지인 M1맥/클라우드맥/CI/새 Apple Silicon맥 필요. **번들ID `ai.jusunsaeng.app` iOS·Android 완전 통일 완료**(Android는 자바 패키지 디렉토리까지 이동, 2026-07-31). **결정(2026-07-31)**: 친구 재우 하차 → 정태민 단독, **안드로이드 앱 1개 Google Play 출시로 마무리**(선택지 A). 이 맥+Android Studio로 가능(에뮬 확인, iOS는 Apple Silicon 필요라 제외). **🎉 에뮬 실행 성공(2026-07-31)**: Pixel 8/API 34 x86_64에서 앱 완전 작동 — debug APK 5.0MB, **앱 내 RAG 답변까지 실증**. 과정에서 **CORS 실버그 1건 발견·수정**(Android 14 WebView origin이 `https://localhost`라 차단 → `https?://localhost`로 확장, logcat 실측이 기존 문서 가정을 뒤집음). **main 머지 완료(2026-08-01, `ca45449`)** — `/privacy` 라이브 404→200으로 **Play용 개인정보처리방침 URL 확보**, 백엔드 911 passed. 콜드스타트 UX 개선 `fix/coldstart-ux` **머지 완료**(2026-08-03 `f2faf09`). **서명 배선 + AAB 파이프라인 검증 완료(2026-08-20, `chore/android-signing-prep` `7867fc7` 미푸시)** — 조건부 릴리스 서명(`key.properties` 없으면 미서명 폴백), **`.gitignore`에 keystore/key.properties 누락 발견·수정**(Capacitor 템플릿이 주석처리 상태여서 서명 비번이 커밋될 수 있었음, 더미파일 실측 검증), 아이콘 1024, `bundleRelease` 드라이런 성공(3.6MB 미서명 AAB·appId·앱명·privacy 확인). 서명 배선은 **PR #123 머지 완료**(2026-08-20 `3083cb2`, CI green, origin/main 시크릿 0 확인). **목표 = 프로덕션 정식 출시 완주**(2026-08-21 확정 — 사용자: "이거 자체를 제대로 끝내버리고 싶다". 내부 테스트는 폴백일 뿐, "Google Play 출시"라 쓸 수 있느냐의 급 차이). **테스터 12명/14일 요건**: 형식은 "12명 14일 연속 옵트인"이나 승격 심사에서 **실제 사용 지표를 대조**하므로 **설치만으론 반려**(최다 반려=engagement 부족) → 안전기준 **매일 앱 열기**, **삭제 시 시계 리셋**, 12명이 같은 창에 겹쳐야 하므로 **15~16명 여유 확보**. **유료 대행 ❌**(2026년 집중단속·계정 영구정지 리스크). **⚠️ 지인 풀 매우 제한적**(대학 동기·선후배 없음, 재우 없음 — 회사 오리누 동료+가족뿐이라 갤럭시로 한정하면 12명 사실상 불가) → **품앗이 커뮤니티가 주력·지인은 보조**. **🎯 모집 채널 조사 결과(2026-08-21)**: ⭐**테스트잇 `test.it.kr`** — 무료 크레딧 품앗이인데 **안드로이드 폰 없이 "웹사이트 테스트"(PC 검수)로 크레딧 획득 가능** = "갚을 기기가 없다"는 문제를 우회하는 유일한 자립 수단(**앞선 "아이폰이라 품앗이 불가" 판단 정정**). **반드시 "B 연속형"(14일 3회 인증) 모집** — A 일반형은 요건 미충족. 규모 작아 병행 필요. ⭐**네이버 카페 `cafe.naver.com/devsharing`**("안드로이드 비공개 테스터" 검색) — **어느 개발자는 반나절에 12명 전원 모집** 실증, 크레딧 강제 아닌 부탁문화라 "앱은 못 깔아드리지만 웹/QA로 돕겠다" 제안 여지. 3순위=카톡 오픈채팅·`테스터 품앗이` 앱·LaunchPad(초기단계). 4순위=Reddit `r/AndroidClosedTesting`. **⚠️실전 함정: "이메일 등록만 하면 된다"고 착각한 팀이 "다운로드 없으면 승격 거절" 뒤늦게 알고 전원 재연락** → **등록→옵트인→설치→매일 사용이 한 세트**. **남은 것: keystore(사용자 직접·비번 입력) → 서명 AAB → 스크린샷 → Play $25 → 비공개 테스트 업로드 → 테스터 12명 14일 → 프로덕션 승격.** 상세: CLAUDE.local.md "🔑 테스터 12명/14일 요건". **보안점검 all green**(앱 번들에 시크릿 전무·프로덕션 JWT_SECRET 실증). 출시 절차 11단계는 `frontend/ANDROID_RELEASE.md`. 유지비 월 $5~15(Railway+OpenAI). 상세: CLAUDE.local.md "모바일 앱화" + `frontend/ANDROID_RELEASE.md` + memory/project_ai_agent_ios_app
...
_Last Updated: 2026-08-21 세션후반 (**테스터 모집 채널 조사 — 아이폰 유저도 참여 가능한 경로 발견 + 지인 풀 정정**). 코드 변경 없음(조사·문서). PR **#124**(요건 조사·프로덕션 확정) → **#125**(채널 조사·지인풀 정정) 머지 완료, 둘 다 CI green. **① ⚠️두 건 정정**: (a) **"아이폰 유저라 품앗이 참여 불가"는 틀렸음** — **테스트잇 `test.it.kr`은 안드로이드 폰 없이도 "웹사이트 테스트"(PC에서 남의 사이트 검수·버그 리포팅)로 크레딧 획득 가능** → "갚을 기기가 없다"는 문제를 우회하는 실질적 자립 수단이 존재. 태민님 QA·버그찾기 강점과도 맞음. (b) **동원 가능 풀에 대학 동기·선후배와 친구 재우를 넣었던 것은 잘못된 가정** — 사용자 확인 결과 **둘 다 없음**. 남은 건 **회사(오리누) 동료 + 가족·친척**뿐이고 갤럭시로 한정하면 **12명은 사실상 불가능** → **전략 반전: 품앗이 커뮤니티가 주력, 지인은 보조**(당초 판단의 반대). **② 채널 우선순위**: ⭐1 **테스트잇 `test.it.kr`**(무료·가입 1,000P·웹사이트 테스트로 크레딧 → **아이폰 가능**. **반드시 "B 연속형"=14일 3회 인증으로 모집**, A 일반형은 요건 미충족. 플랫폼도 "중간 opt-out 리셋→2~3명 여유" 경고. **단 규모 작음**[앱 모집 4건 수준]이라 단독으론 12명 부족) / ⭐2 **네이버 카페 `cafe.naver.com/devsharing`**("안드로이드 비공개 테스터" 검색 — **어느 개발자는 반나절에 12명 전원 모집** 실증, 크레딧 강제가 아닌 부탁·품앗이 문화라 "제 앱은 못 깔아드리지만 웹/QA로 돕겠습니다" 제안이 통할지 문의 가치) / 3 카톡 오픈채팅("구글 플레이 테스터"·"테스터 품앗이")·`테스터 품앗이` 앱(남의 앱 테스트하면 내 글이 "맞품앗이"로 최상단)·LaunchPad(apptesters.cc, 무료·SDK 자동추적이나 **"No apps registered yet" 초기단계**로 실효성 낮음) / 4 해외 Reddit `r/AndroidClosedTesting`(전용 서브레딧·상호교환)·`r/androiddev`·Discord(영어·품질 편차) / **❌ 크몽·Fiverr·Upwork 저가 대행**(에뮬·봇 계정 위험 → 계정 영구정지). **③ ⚠️실전 함정 추가(실사례)**: 어느 팀은 **"테스터 이메일 등록만 하면 된다"고 착각**했다가 "**다운로드 수가 없으면 프로덕션 출시 권한 신청이 거절될 수 있다**"를 뒤늦게 알고 **전원에게 다시 연락해 설치를 부탁**해야 했음(velog @bcgo99) → **등록 → 옵트인 → 설치 → 매일 사용**까지가 한 세트. 지인 부탁이 심리적으로 부끄럽다는 후기가 공통적 → 품앗이 커뮤니티는 서로 같은 처지라 이 부담이 없는 게 실질 장점. **④ 다음주 실행안**: **①테스트잇 가입 → 웹사이트 테스트로 크레딧이 실제로 모이는지 검증**(이 계획 전체의 전제이고 아직 문서로만 확인한 상태라 **여기부터 실증 필요**) → ②네이버 카페 가입·분위기 파악·아이폰 참여 가능 여부 문의 → ③병행으로 keystore→서명AAB→Play $25→**비공개 테스트 트랙 업로드**(=12명 카운터 시작) → ④카페+테스트잇+회사동료/가족 합쳐 **15~16명**(단일 채널 의존 금지, 1명 이탈이면 리셋). 상세: CLAUDE.local.md "📍 모집 채널 조사"·`frontend/ANDROID_RELEASE.md` 10단계)_

_2026-08-21 (**테스터 12명/14일 요건 조사 → 프로덕션 정식 출시로 확정**). 코드 변경 없음(조사·의사결정). **① PR #123 머지 완료**(`3083cb2`, CI green test 2m38s·rag-eval pass, 브랜치 삭제, **origin/main 시크릿 0 실측**: 템플릿만 추적·비번 필드 빈 값). **② 테스터 요건은 2층 구조**(공식문서+실무자료 확인): 형식요건은 "12명 14일 연속 옵트인"뿐이나, 14일 후 **프로덕션 접근 신청서**에서 "테스터가 모든 기능을 썼는지"를 묻고 **심사자가 Play Console 실제 지표와 대조** → **설치만으론 반려**(최다 반려사유 = insufficient testing engagement, "옵트인 12명 + 세션 0"은 확정 반려). **안전기준 = 14일간 매일 앱 열기**. 리셋 함정: **앱 삭제 시 옵트인 카운트 감소 → 14일 시계 리셋**(가장 치명적), 12명이 **같은 14일 창에 겹쳐야**(누적 12명 ❌, 7일차에 1명 이탈하면 리셋) → **15~16명 여유 확보가 정석**. 부탁 문구: "깔아만 놔줘" ❌ → **"2주간 하루 한 번 열고 절대 지우지 마"** ⭕(주식 앱이라 "관심종목 등록해두고 장 열릴 때 확인"으로 자연스럽게 포장됨). **③ 유료 대행 업체 = 쓰지 않기로**: 시장 존재($15~30, 수시간 내 12명)하나 Google이 2026년 집중 단속(Play Services 하드웨어 검사로 에뮬 플래그 — x86의 ARM 위장·배터리 무감소·가속도계 부재) → 걸리면 **프로덕션 거부 + 개발자 계정 영구 정지**. 업체의 에뮬 사용 여부는 발주자가 확인 불가인데 대가는 발주자가 치름 = **$20 아끼려 계정을 거는 거래**. 게다가 대행 테스터는 앱에 관심 없어 engagement 부족으로 반려되는 최악 조합이 흔함. **④ 품앗이(상호 테스트)는 정책상 문제없으나 태민님은 참여 불가**: 네이버 카페·오픈카톡·Reddit `r/androiddev` 등에 존재하고 실제 사람+실제 기기라 단속 대상이 아님(상대도 자기 앱을 통과시켜야 해서 성실 — engagement에 오히려 강함). 그런데 **아이폰 유저라 남의 안드로이드 앱을 설치할 기기가 없어 갚을 수 없음**(에뮬레이터는 카운트 불가) → 받는 쪽만 하면 커뮤니티가 걸러내고 결국 유료 대행과 같은 위치로 회귀. **→ 유일한 정직한 경로 = 갤럭시 쓰는 지인 12명**(한국 갤럭시 점유율 60~70%라 지인 20명 중 12~14명 가능성. 회사 오리누 동료·가족·대학 동기·친구 재우[앱화는 하차했으나 테스터로는 가능]). **⑤ ⚠️ 직전 세션 판단 2건 정정**: (a) 8-20에 내부테스트 vs 프로덕션 차이를 **"검색 노출 하나만 없다"고 정리한 것은 안이했음** — 사용자 지적대로 **"Google Play 출시"라고 쓸 수 있느냐**의 급 차이이고, 내부테스트로 끝내면 면접에서 "왜 정식 출시는 안 하셨어요?"를 유발(답이 기술 이유가 아니라 인맥 이유라 궁색). (b) 8-20에 **"14일 대기"와 "다른 프로젝트 개발"을 대립시킨 것도 틀림** — 14일은 대기 시간이고 매일 앱을 여는 건 지인들이 하므로 **충돌하지 않음**. **⑥ ✅ 결정: 프로덕션 정식 출시 완주**(사용자: "포트폴리오는 급하지 않지만 이거 자체를 제대로 끝내버리고 싶다"). **핵심 통찰: 다음주 할 작업은 어느 쪽을 택하든 동일** — keystore → 서명 AAB → Play $25 → 업로드까지 같고, 업로드를 **비공개 테스트(closed testing) 트랙**으로 하면 그 시점부터 12명 카운터가 돌기 시작 → **지금 결정할 필요 없이 업로드까지 밀어붙이면 됨**. 병행: 갤럭시 지인 명단 세어보기(15~16명 목표). 정말 안 되면 그때 내부 테스트로 선회(손실 0, **$25는 평생 1회·유효기간 없어 나중 승격도 가능**). 상세: CLAUDE.local.md "🔑 테스터 12명/14일 요건"·memory/project_ai_agent_ios_app)_

_2026-08-20 (**릴리스 서명 배선 + AAB 파이프라인 검증 / 마무리 방향 확정**). 17일 공백 후 복귀 — 그사이 커밋 17개는 **전부 자동 데이터 수집**(신규 개발 0). **복귀 점검 all green**: 라이브 백엔드·프론트·`/privacy` 200, 데이터 최신일 **20260819**(ETF 769·주식 1843), GitHub Actions 자동수집 **5/5 success** → **17일 무개입으로 운영 유지 실증**(포트폴리오상 "만들었다"보다 "운영했다"의 근거). **① ⚠️ 보안 이슈 1건 실제 발견·수정**: Capacitor 안드로이드 템플릿의 `.gitignore`가 `#*.jks`/`#*.keystore`를 **주석 처리**한 상태이고 `key.properties`는 항목 자체가 없어서, 그대로 서명 배선했으면 **릴리스 서명 비밀번호가 공개 리포에 커밋**될 수 있었음(유출 시 제3자가 이 앱으로 위장한 앱에 서명 가능). 주석 해제 + `key.properties` 추가 후 **더미 파일로 `git check-ignore` 실측 검증**(가정 아님), 커밋 직전 스테이징 grep으로 혼입 0 재확인. **② 조건부 서명 배선**(`android/app/build.gradle`): `key.properties`가 있으면 릴리스 서명, **없으면 미서명 빌드로 폴백** — 무조건 서명하면 keystore 없는 로컬/CI 빌드가 전부 깨지므로. `key.properties.example` 템플릿 신설(`storeFile`은 절대경로 — gradle은 `~` 미확장). **③ AAB 드라이런 성공**: `build:static`→`cap sync android`→`./gradlew bundleRelease` **BUILD SUCCESSFUL(57s)**, **3.6MB**, appId `ai.jusunsaeng.app`·앱명 `주선생`·`/privacy` 포함 확인 → **keystore만 꽂으면 서명 AAB 한 방**. ⚠️**AAB에 서명 블록 없는 건 정상**(`META-INF/*.RSA` 부재를 서명실패로 의심했으나, AAB는 APK와 서명구조가 다르고 keystore 없으니 `signReleaseBundle`이 no-op — 즉 **디버그 키로 몰래 서명되지 않았다는 뜻**이라 의도된 동작. 디버그 서명 AAB는 Play가 거부. `base/root/META-INF/*.version`은 라이브러리 버전파일로 서명과 무관, 오독 금지). **④ 아이콘 1024 생성**(512 업스케일 상태 해소). **⑤ 회귀 all green**: 백엔드 **911 passed**·프론트 **22 passed**·tsc 0·`out/privacy.html` + 라이브 백엔드 URL 주입 확인. **⑥ 마무리 방향 확정(재논의 방지)**: **배포+블로그 9편으로 프로젝트 마무리 = 찬성**(Phase 0~F 완료·운영 실증으로 완결 조건 충족, 남은 기능 후보는 이력 한 줄이 새로 안 생김. **단 테스터 12명 14일이 진짜 관문** — 못 모을 것 같으면 내부테스트까지만 하고 블로그를 "제출까지의 전 과정"으로 쓰는 것도 완결로 충분, 흐지부지가 최악). **GitHub private 전환 = 반대**(포트폴리오는 남이 봐야 가치 있고 블로그 8편이 코드를 가리킴 / 시크릿은 이미 Railway에만 있고 유일 구멍이던 keystore를 이번에 막음). **배포 전 리팩터링 = 불필요**(7-10 #110에서 이미 한 라운드, 이후 신규 코드 없음, 출시 직전 리팩터링은 리스크만 — "빌드되는 상태를 건드리지 않는다"). 다음(다음주): 브랜치 푸시·머지 → **keystore 생성(사용자 직접, 📸 1회성 터미널 캡처 먼저)** → 스크린샷 → Play $25 → 테스터 12명 14일 → AAB 제출 → 블로그 9편. 상세: CLAUDE.local.md·`frontend/ANDROID_RELEASE.md`·memory/project_ai_agent_ios_app)_

_2026-08-01 (**안드로이드 에뮬 실행 성공 + CORS 실버그 + main 머지 + 콜드스타트 UX**). **① 🎉 앱이 실기기(에뮬)에서 완전 작동 실증**(7-31 16:36~17:36): Android Studio + Pixel 8/API 34 x86_64 에뮬 → ▶Run → debug APK 5.0MB → **에뮬 앱에서 "삼성전자 기술적 분석" RAG 정상 답변**(MA/RSI 33.6/MACD/볼린저). Intel 맥에서 Capacitor 안드로이드 빌드 가능 확인(iOS는 CPU 제약으로 여전히 불가). 환경 함정: AS 기본 JVM 25가 Gradle 8.14.3과 비호환→**"Use JVM 21" 원클릭**, Device Manager는 상단 드롭다운에 없음→**Double Shift 검색**, AS 업그레이드 권유는 전부 무시("빌드되는 상태 건드리지 않는다"). **② CORS 실버그 — 기존 문서 가정이 틀렸음**: 앱에서 "연결 오류" → 에뮬 네트워크/Chrome은 정상 → **`adb logcat`이 결정적 증거**로 `origin 'https://localhost' blocked by CORS`. **Android 14 최신 WebView origin은 `https://localhost`(s 붙음)** 인데 코드는 `http://localhost`만 허용했던 것 → regex `^(capacitor://localhost|https?://localhost)$`로 확장. **교훈: WebView origin 같은 런타임 값은 훈련지식/기존 메모리가 아니라 logcat 실측이 진실**(이번은 에이전트 오판이 아닌 진짜 버그·로그로 확증). 배포는 main에 Capacitor CORS 코드 자체가 없어 cherry-pick 충돌→abort→**main에 직접 이식**(`cdc36a5`), 반영에 약 5분. **③ feat/ios-capacitor→main 머지 완료**(`ca45449`): `/privacy`가 브랜치에만 있어 **라이브 404**였고 Play 제출 필수 URL이라 머지가 선행조건. 충돌 2건 해소(위 CORS 이중커밋이 원인) — 미사용 상수 제거·테스트 합집합. ⚠️`git merge-tree` 출력 오독으로 "충돌 없음" 오판(실제 `git merge`가 진실). **백엔드 911 passed**(895→911)·프론트 tsc0/Vitest17·build:static **16라우트**·`cap sync android`로 앱 번들 반영. **`/privacy` 404→200 라이브 확인 → ✅Play용 개인정보처리방침 URL 확보**. 이후 작업은 main에서. **④ 콜드스타트 UX 개선**(브랜치 `fix/coldstart-ux` `b10ab89`, **미배포**): Railway 유휴 재시작 중 502를 즉시 "연결 오류"로 끝내던 것을 **4초×5회 자동 재시도 + "서버를 깨우고 있어요" 진행표시**로(에러 아님). 토큰 수신 후엔 재시도 금지(중복방지)·abort 연동·문구 "백엔드"→"서버". **검증 2층**: mock 4개 + **실제 HTTP 1개**(가짜 백엔드 502·502·200에 진짜 접속) → 프론트 **22 passed**. **⑤ 📸 블로그 9편 스크린샷 체크리스트 신설**(포스팅 완료까지 유지): 1회성 최우선 = Play $25결제·keystore 터미널·테스터12명 카운터·AAB제출·스토어 등록페이지. **⑥ 다음 ToDo**: keystore 생성(사용자 직접 — 비번 로그 노출 방지·분실 시 업데이트 영구불가) → 서명배선/AAB → coldstart 배포 → Play $25 → 테스터 12명 14일. 상세: CLAUDE.local.md·memory/project_ai_agent_ios_app·project_ai_agent_blog9_screenshots)_

_2026-07-31 (재우 하차→정태민 단독, **안드로이드 앱 1개 Google Play 출시로 결정**(선택지 A). **Android 번들ID `ai.jusunsaeng.app` 통일 완료**(파일4곳+자바 패키지 디렉토리 이동, iOS와 통일). **출시 체크리스트 `frontend/ANDROID_RELEASE.md` 신설**(11단계, keystore 백업 강조). **보안점검 all green(실증)**: 앱 정적번들에 백엔드 시크릿 전무·클라 노출은 NEXT_PUBLIC_API_BASE뿐 / 실제.env 미추적 / 비번 bcrypt / 프로덕션 JWT_SECRET은 dev기본값 아님(위조토큰 라이브 401 실증) / cron X-Cron-Token 보호 → **스토어 출시 보안 문제없음**, 주의는 keystore 백업뿐. 유지비 월 $5~15(사용자 적을 때, Railway 상주+OpenAI 종량, 나머지 무료). Play $25 평생1회. **개인정보 동의 절차 구현 완료(세션후반)**: 개인정보처리방침 페이지 `/privacy`(정적 16라우트) + 회원가입 동의 체크박스(미동의 3중 차단). 성별=필수 유지·추가 수집 안 함(최소수집). 로그인 선택제 유지. build/tsc/렌더 검증. 상세: CLAUDE.local.md·ANDROID_RELEASE.md)_

_2026-07-27 (앱 배포 방향 재정립 + Tahoe 업글 후 재검증. **iOS 빌드 불가 확정(정정)**: macOS 14→26.5.2 Tahoe 업글했으나 최신 Xcode 26.6은 **Apple Silicon 전용**(App Store "M1 이상 필요" 실물확인) → Intel 2019 맥은 CPU 때문에 설치 불가. 관문은 macOS버전 아닌 CPU였음(앞선 "Tahoe면 iOS 열림" 오류). iOS는 지인M1맥/클라우드맥/CI/새맥 필요. **번들ID `ai.jusunsaeng.app` 확정**(iOS 3곳 반영·잔여없음, Android는 8월 통일). **방향: PWA로 아이폰 사용($0) + 갤럭시 배포 예상 8월**(둘 다 아이폰이라 안드 먼저면 본인들 못 써보는 딜레마 인지). **서비스 재검증 all green**: 라이브 백/프론트 200, 챗봇 RAG(/chat 필드=`question`) 정확답변, 데이터 기준일 07-23·ETF749/주식1841, 자동수집 매일success(7/18~23), 방문자 누적256, 로컬 188 passed(Tahoe OK). ⚠️KRX 비번만료 ~10월초 예정. 상세: CLAUDE.local.md·memory/project_ai_agent_ios_app)_

_2026-07-15 (모바일 앱화 — Capacitor v8로 iOS+Android 래핑, 브랜치 feat/ios-capacitor 미머지. output:"export" 정적번들 기기내장·server.url無로 Apple 4.2 회피·웹/앱 한 코드베이스(BUILD_TARGET)·SPM(CocoaPods불필요)·manifest force-static·OfflineBanner/NativeAppInit·CORS는 iOS capacitor://localhost+Android http://localhost 둘 다 regex. 앱명 "주선생". build 양쪽+백56/프17 green. 상세: CLAUDE.local.md·frontend/IOS_APP.md·memory/project_ai_agent_ios_app)_
