...
def _coerce_name(name) -> str:
    ...
    pykrx 내부 DataFrame에 ticker가 중복되면 `get_market_ticker_name()`이
    string 대신 pandas Series를 반환 → SQLite 바인딩에서
    'type Series is not supported' 발생 (2026-06-01 daily-collect 장애).
    Series면 첫 값만 추출.
    """
    if name is None:
        return ""
